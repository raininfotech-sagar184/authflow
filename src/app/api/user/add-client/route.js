...
export async function POST(req, res) {
    try {
        ...
        if (userId) {
            ...
            let getuser = await sql_query("select userId,isClient from tbluserDetail where userId = ? ", [id])
            if (getuser) {
                if (getuser.isClient == 1) {
                    return NextResponse.json({ message: "User is already client" }, { status: 400 })
                } else {
                    let now = get_timestemp()
                    await sql_query("update tbluserDetail set isClient = ?,updatedOn = ? where userId = ?", [1, now, getuser.userId])
