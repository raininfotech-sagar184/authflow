...
export async function POST(request, response) {
    try {
        ...
        const { name, amount, rewarType,id,imageUrl,isImage,status } = body
        try {
            validate_string(name, "Package name")
            validate_input_number(amount, "Package amount")
            validate_input_number_in_range(status, 'Status') 
        ...
        if (id) { 
            await sql_query(`UPDATE tblPackage set name=?,amount=?,rewardType=?,image=?,status=?,updatedOn=? WHERE packageId=?`, [name,parseFloat(amount),parseInt(rewarType),newImageName,status,currentTime,dec(`${id}` , encryption_key("ids"))] )
        } else {
            await sql_query(`INSERT into tblPackage (name,amount,rewardType,image,status,createdOn) VALUES (?,?,?,?,?,?)`, [name,parseFloat(amount),parseInt(rewarType),newImageName,status,currentTime], "Insert")
         }
