...
export default function Header() {
  const { pageLoader, setAuthTkn, loginUserData } = useAuthContext()
  ...
  return (
    <>
      {/* <!-- Navbar: Start --> */}
      <nav className={`layout-navbar shadow-none py-0 ${pageLoader ? 'd-none' : ''}`} onClick={() => console.log(loginUserData)} onMouseLeave={() => setDropShow(false)}>
            ...
            <ul className="navbar-nav flex-row align-items-center ms-auto">
...

              {
                (loginUserData && loginUserData.userName) ? (<>
                  ...
                  <span>
                    <button href="../vertical-menu-template/auth-login-cover.html" className="btn btn-primary" target="_blank" onClick={(e) => logout(setLoader)}>
                      {loader ?<i className="ti ti-spin ti-refresh"></i> : <span className="tf-icons ti ti-login scaleX-n1-rtl me-md-1"></span>}
                  ...
                  <span>
                    <Link href="login" className="btn btn-primary" target="_blank">
